...
CREATE INDEX IF NOT EXISTS idx_refresh_tokens_active ON refresh_tokens (user_id, user_type, revoked, expires_at);

-- Add trigger to update last_used_at when token is validated
DROP TRIGGER IF EXISTS update_refresh_token_last_used;
DELIMITER //
